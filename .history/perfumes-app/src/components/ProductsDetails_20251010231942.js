                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (!product || !product._id) {
                        setAlert({ message: "Product not loaded yet!", type: "danger" });
                      ...
                      if (!token) {
                        setAlert({ message: "Please login first!", type: "danger" });
                        setTimeout(() => setAlert(null), 3000);
                        return;
                      }
                      console.log("Token 2 being sent:", token);

                      try {
                        // Using axios is safer for JSON parsing and error handling
                        ...
                        // Update cart UI
                        if (addToCart) addToCart(res.data.items || []);

                        // Show success alert
                        setAlert({ message: `${product.name} added to your cart!`, type: "success" });
                        setTimeout(() => setAlert(null), 3000);
                      } catch (err) {
                        console.error("Add to cart error:", err.response?.data || err.message);
                        setAlert({ message: "Error adding to cart", type: "danger" });
                        setTimeout(() => setAlert(null), 3000);
                      }
                    }}
                  >
